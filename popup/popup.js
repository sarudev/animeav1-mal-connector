...
function renderSearchResults(results) {
  ...
  results.forEach((anime) => {
    ...
    const episodesText = anime.num_episodes
...
async function checkConnection() {
  ...
  if (res && res.ok) {
    els.connectionStatus.textContent = `✅ Conectado a MAL como ${
      res.user?.name ?? ''
    }`;
    els.connectionStatus.className = 'status ok';
  } else {
    els.connectionStatus.textContent = `⚠️ Sin conexión con MAL: ${
...
els.manualIdInput.addEventListener('input', () => {
  if (els.manualIdInput.value.trim()) {
    state.selectedResult = null;
    document
      .querySelectorAll('input[name="search-result"]')
...
els.confirmLinkBtn.addEventListener('click', async () => {
  ...
  const pictureUrl = manualId
    ? null
    : state.selectedResult?.main_picture?.medium || null;
